...
/**
 * Semistructuredstore struct that wraps query and compress functions for boost python
 */
struct Semistructuredstore{
    // Constructor that loads from semistructured 
    Semistructuredstore(std::string filename) {
        s_file_ = nullptr;
    ...
    Semistructuredstore(const std::string& inputpath, uint32_t sa_sampling_rate, uint32_t isa_sampling_rate,
    uint32_t npa_sampling_rate, int sampling_opt, int npa_opt){
        ...
        s_file_ = new SuccinctSemistructuredShard(inputpath,
                                    ...
                                    npa_sampling_rate, 3, SamplingSchemeFromOption(sampling_opt),
    ...
    //Semistructuredstore members
    SuccinctSemistructuredShard *s_file_;

};

//Boost Python module
BOOST_PYTHON_MODULE(semistructuredstore){
  std::string  (Semistructuredstore::*g1)(int64_t) = &Semistructuredstore::Get;
  std::string  (Semistructuredstore::*g2)(int64_t, std::string&) = &Semistructuredstore::Get;

  class_<Semistructuredstore>("Semistructuredstore", init<std::string>())
    .def(init<std::string, uint32_t, uint32_t, uint32_t, int, int>())
    .def("Search", &Semistructuredstore::Search)
    .def("Count", &Semistructuredstore::Count)
    .def("Get", g1)
    .def("Get", g2)
    ;
