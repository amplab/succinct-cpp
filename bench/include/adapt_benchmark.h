...
class AdaptBenchmark : public Benchmark {
 public:
  AdaptBenchmark(std::string configfile, std::string reqfile,
                 std::string resfile, std::string addfile, std::string delfile,
                 double skew, uint32_t len, uint32_t batch_size,
  ...
  void run_benchmark() {
    std::thread req(&AdaptBenchmark::send_requests, query_client,
                    query_transport, queries, request_rates, durations,
                    std::ref(queue_length), reqfile);
