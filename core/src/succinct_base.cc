#include "../include/succinct_base.h"
...
size_t SuccinctBase::memorymap_vector(std::vector<uint64_t> &v, uint8_t* buf) {
...
void SuccinctBase::init_bitmap(SuccinctBase::BitMap **B, uint64_t size_in_bits,
...
size_t SuccinctBase::dictionary_size(Dictionary *D) {
  ...
  return sizeof(D->size)
      + 2 * ((D->size / L3BLKSIZE) + (D->size / L2BLKSIZE) + 2)
          * sizeof(uint64_t) + bitmap_size(D->B);
}
