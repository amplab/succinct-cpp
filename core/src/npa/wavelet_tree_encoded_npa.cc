...
void WaveletTreeEncodedNPA::encode(bitmap_t *data_bitmap, bitmap_t *compactSA,
  ...
  for (uint64_t i = 1; i < npa_size; i++) {
    ...
    npa_val = SuccinctBase::lookup_bitmap_array(
        compactISA,
        (SuccinctBase::lookup_bitmap_array(compactSA, i, logn) + 1) % npa_size,
        logn);
...
uint64_t WaveletTreeEncodedNPA::operator[](uint64_t i) {
  ...
  uint64_t num_contexts =
      (row_id > 0) ? csizes[row_id] - csizes[row_id - 1] : csizes[0];

  // Get context offset
  uint64_t context_off = SuccinctBase::get_rank1(&(row_nec[row_id]), column_id)
      - 1;
