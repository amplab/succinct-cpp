    ...
    case SuccinctMode::LOAD_IN_MEMORY: {
      ...
      switch (sa_sampling_scheme) {
        ...
        case SamplingScheme::LAYERED_SAMPLE_BY_INDEX:
          SA = new LayeredSampledSA(sa_sampling_rate,
                                    sa_sampling_rate * sampling_range, npa,
                                    s_allocator);
          break;
        case SamplingScheme::OPPORTUNISTIC_LAYERED_SAMPLE_BY_INDEX:
          SA = new OpportunisticLayeredSampledSA(
              sa_sampling_rate, sa_sampling_rate * sampling_range, npa,
              s_allocator);
          break;
        default:
          SA = NULL;
      ...
      switch (isa_sampling_scheme) {
        ...
        case SamplingScheme::FLAT_SAMPLE_BY_VALUE:
          ...
          break;
        case SamplingScheme::LAYERED_SAMPLE_BY_INDEX:
          ISA = new LayeredSampledISA(isa_sampling_rate,
                                      isa_sampling_rate * sampling_range, npa,
                                      s_allocator);
          break;
        case SamplingScheme::OPPORTUNISTIC_LAYERED_SAMPLE_BY_INDEX:
          ISA = new OpportunisticLayeredSampledISA(
              isa_sampling_rate, isa_sampling_rate * sampling_range, npa,
              s_allocator);
          break;
        default:
          ISA = NULL;
...
void SuccinctCore::construct(const char* filename, uint32_t sa_sampling_rate,
  ...
  divsufsortxx::constructSA((uint8_t *) data, (uint8_t *) (data + input_size),
                            lSA, lSA + input_size, 256);

  // Compact SA
  BitMap *compactSA = new BitMap;
  create_bitmap_array(&compactSA, (uint64_t *) lSA, input_size, bits,
                      s_allocator);
  ...
  switch (npa_encoding_scheme) {
    case NPA::NPAEncodingScheme::ELIAS_GAMMA_ENCODED:
      npa = new EliasGammaEncodedNPA(input_size, alphabet_size, context_len,
                                     npa_sampling_rate, data_bitmap, compactSA,
                                     compactISA, s_allocator);
      break;
    case NPA::NPAEncodingScheme::ELIAS_DELTA_ENCODED:
      npa = new EliasDeltaEncodedNPA(input_size, alphabet_size, context_len,
                                     npa_sampling_rate, data_bitmap, compactSA,
                                     compactISA, s_allocator);
      return;
    case NPA::NPAEncodingScheme::WAVELET_TREE_ENCODED:
      npa = new WaveletTreeEncodedNPA(input_size, alphabet_size, context_len,
                                      npa_sampling_rate, data_bitmap, compactSA,
                                      compactISA, s_allocator, this);
      break;
    default:
      npa = NULL;
  }
  ...
  switch (sa_sampling_scheme) {
    case SamplingScheme::FLAT_SAMPLE_BY_INDEX:
      SA = new SampledByIndexSA(sa_sampling_rate, npa, compactSA, input_size,
                                s_allocator);
      break;
    case SamplingScheme::FLAT_SAMPLE_BY_VALUE:
      SA = new SampledByValueSA(sa_sampling_rate, npa, compactSA, input_size,
                                s_allocator, this);
      break;
    case SamplingScheme::LAYERED_SAMPLE_BY_INDEX:
      SA = new LayeredSampledSA(sa_sampling_rate,
                                sa_sampling_rate * sampling_range, npa,
                                compactSA, input_size, s_allocator);
      break;
    case SamplingScheme::OPPORTUNISTIC_LAYERED_SAMPLE_BY_INDEX:
      SA = new OpportunisticLayeredSampledSA(sa_sampling_rate,
                                             sa_sampling_rate * sampling_range,
                                             npa, compactSA, input_size,
                                             s_allocator);
      break;
    default:
      SA = NULL;
  ...
  switch (isa_sampling_scheme) {
    case SamplingScheme::FLAT_SAMPLE_BY_INDEX:
      ISA = new SampledByIndexISA(isa_sampling_rate, npa, compactSA, input_size,
                                  s_allocator);
      break;
    case SamplingScheme::FLAT_SAMPLE_BY_VALUE:
    ...
    case SamplingScheme::OPPORTUNISTIC_LAYERED_SAMPLE_BY_INDEX:
      ISA = new OpportunisticLayeredSampledISA(
          isa_sampling_rate, isa_sampling_rate * sampling_range, npa, compactSA,
          input_size, s_allocator);
      break;
    default:
      ISA = NULL;
  }
...
size_t SuccinctCore::deserialize() {
  ...
  for (uint64_t i = 0; i < alphabet_map_size; i++) {
    ...
    alphabet_map[first] = std::pair<uint64_t, uint32_t>(second_first,
                                                        second_second);
  }
...
std::pair<int64_t, int64_t> SuccinctCore::bw_search(std::string mgram) {
  ...
  if (alphabet_map.find(mgram[m_len - 1]) != alphabet_map.end()) {
    ...
    range.second = alphabet_map[alphabet[alphabet_map[mgram[m_len - 1]].second
        + 1]].first - 1;
  } else
  ...
  for (int64_t i = m_len - 2; i >= 0; i--) {
    if (alphabet_map.find(mgram[i]) != alphabet_map.end()) {
      ...
      col_range.second =
          alphabet_map[alphabet[alphabet_map[mgram[i]].second + 1]].first - 1;
    } else
      return std::pair<int64_t, int64_t>(0, -1);
...
std::pair<int64_t, int64_t> SuccinctCore::continue_bw_search(
...
std::pair<int64_t, int64_t> SuccinctCore::continue_fw_search(
