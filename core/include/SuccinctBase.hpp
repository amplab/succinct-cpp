...
#include "utils/SuccinctAllocator.hpp"
#include "utils/SuccinctUtils.hpp"
#include "utils/buf_allocator.hpp"
...
public:
    ...
    static void create_bitmap_array(BitMap **B, uint64_t *A, uint64_t n, uint32_t b, SuccinctAllocator s_allocator);

    // Set a value in the bitmap, treating it as an array of fixed length
    static void set_bitmap_array(BitMap **B, uint64_t i, uint64_t val, uint32_t b);
    ...
    static void set_bitmap_pos(BitMap **B, uint64_t pos, uint64_t val, uint32_t b);
