...
#include "utils/SuccinctUtils.hpp"
...
class DeltaEncodedNPA : public NPA {
 public:
  ...
  virtual size_t memorymap_delta_encoded_vector(DeltaEncodedVector *dv,
                                                uint8_t *buf) {
 ...
 public:
  ...
  virtual void encode(bitmap_t *data_bitmap, bitmap_t *compactSA,
    ...
    npa_val = SuccinctBase::lookup_bitmap_array(
        compactISA,
        (SuccinctBase::lookup_bitmap_array(compactSA, 0, logn) + 1) % npa_size,
        logn);
  ...
  virtual uint64_t operator[](uint64_t i) {
    ...
    uint64_t npa_val = lookupDEV(&(del_npa[column_id]),
                                 i - col_offsets[column_id]);
    return npa_val;
  }

  virtual size_t storage_size() {
    size_t tot_size = 3 * sizeof(uint64_t) + 2 * sizeof(uint32_t);
    tot_size += sizeof(contexts.size())
        + contexts.size() * (sizeof(uint64_t) * 2);
  ...
  virtual size_t serialize(std::ostream& out) {
    ...
    out.write(reinterpret_cast<const char *>(&(sampling_rate)),
              sizeof(uint32_t));
    out_size += sizeof(uint32_t);
  ...
  virtual size_t memorymap(std::string filename) {
    ...
    del_npa = new DeltaEncodedVector[sigma_size];
    for (uint64_t i = 0; i < sigma_size; i++) {
      data += memorymap_delta_encoded_vector(&del_npa[i], data);
    }
