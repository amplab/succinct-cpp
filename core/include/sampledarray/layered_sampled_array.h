 ...
 public:
  ...
  LayeredSampledArray(uint32_t target_sampling_rate,
    ...
    for (uint32_t i = 0; i < this->num_layers; i++) {
      ...
      SuccinctBase::init_bitmap(&layer_data[i], num_entries * data_bits,
                                s_allocator);
    }
  ...
  inline void get_layer(layer_t *l, uint64_t i) {
    ...
    l->layer_idx = (i / base_sampling_rate) * count[l->layer_id]
      ...
      if (layer_id == this->num_layers - 1) {
        for (uint64_t i = 0; i < num_entries; i++) {
          ...
          if (idx > original_size)
            break;
          SuccinctBase::set_bitmap_array(&layer_data[layer_id], i, at(idx),
                                         data_bits);
        }
      } else {
        for (uint64_t i = 0; i < num_entries; i++) {
  ...
  virtual size_t storage_size() {
    ...
    tot_size += 4 * sizeof(uint32_t)
        + this->num_layers * (sizeof(uint32_t) + sizeof(uint64_t));
