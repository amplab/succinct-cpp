...
#define IS_MARKED_FOR_CREATION(i)   GETBIT((this->LAYER_TO_CREATE), (i))
 ...
 public:
  OpportunisticLayeredSampledArray(uint32_t target_sampling_rate,
                                   ...
                                   uint64_t n, SuccinctAllocator &s_allocator)
      : LayeredSampledArray(target_sampling_rate, base_sampling_rate, SA, n,
                            s_allocator) {
    ...
    for (uint32_t i = 0; i < this->num_layers; i++) {
      ...
      SuccinctBase::init_bitmap_set(&is_layer_value_sampled[i], num_entries,
                                    s_allocator);
    }
  }
  ...
  virtual inline uint64_t get_layer_leq(layer_t *l, int64_t i) {
    ...
    i -= n_hops;
    int64_t l_idx = (i / base_sampling_rate) * count[layer[layer_offset]]
    ...
    while (!EXISTS_LAYER(layer[layer_offset])
      ...
      if (i < 0)
        i += original_size;
      l_idx = (i / base_sampling_rate) * count[layer[layer_offset]]
          + layer_idx[layer_offset];
    }
  ...
  virtual bool is_sampled(uint64_t i) {
    ...
    uint64_t l_idx = (i / base_sampling_rate) * count[layer_id]
        + layer_idx[layer_offset];
    return (i % target_sampling_rate == 0) && EXISTS_LAYER(layer_id)
  ...
  bool store(uint64_t i, uint64_t val) {
    if (i % target_sampling_rate == 0) {
      ...
      !IS_LAYER_VAL_SAMPLED(l.layer_id, l.layer_idx)) {
        SuccinctBase::set_bitmap_array(&layer_data[l.layer_id], l.layer_idx,
                                       val, data_bits);
  ...
  void store_without_check(uint64_t i, uint64_t val) {
    layer_t l;
    get_layer(&l, i);
    SuccinctBase::set_bitmap_array(&layer_data[l.layer_id], l.layer_idx, val,
                                   data_bits);
    SET_LAYER_VAL_SAMPLED(l.layer_id, l.layer_idx);
    this->num_sampled_values++;
  }
  ...
  virtual size_t delete_layer(uint32_t layer_id) {
    ...
    if (EXISTS_LAYER(layer_id)) {
      ...
      SuccinctBase::clear_bitmap(&is_layer_value_sampled[layer_id],
                                 s_allocator);
      for (uint64_t i = 0; i < is_layer_value_sampled[layer_id]->size; i++) {
  ...
  virtual size_t reconstruct_layer(uint32_t layer_id) {
    ...
    if (!EXISTS_LAYER(layer_id)) {
      ...
      SuccinctBase::init_bitmap(&layer_data[layer_id], num_entries * data_bits,
                                s_allocator);
