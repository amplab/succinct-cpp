...
private:
    void compute(RegExResult &res, RegEx *r) {
        ...
        {
            fprintf(stderr, "RegEx blank is a place-holder and should not appear in the expression!\n");
            exit(0);
            break;
        }
    ...
    void regexConcat(RegExResult &concat_res, RegExResult &left, RegExResult &right) {
        ...
        for (left_it = left.begin(), right_it = right.begin(); left_it != left.end() && right_it != right.end(); left_it++) {
          ...
          if (right_it->first == left_it->first + left_it->second)
              concat_res.insert(OffsetLength(left_it->first, left_it->second + right_it->second));
        }
    }

    void regexRepeat(RegExResult &repeat_res, RegExResult &a, RegExRepeatType r_type, int min = -1, int max = -1) {
        switch(r_type) {
