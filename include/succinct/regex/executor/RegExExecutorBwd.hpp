...
public:
    RegExExecutorBwd(SuccinctCore *s_core, RegEx *re): RegExExecutorSuccinct(s_core, re) {}
...
private:
    ...
    void regexConcat(ResultSet &concat_results, RegEx *r, ResultEntry right_result) {
        ...
        case RegExType::Primitive:
        {
            ...
            case RegExPrimitiveType::Mgram:
            {
                Range range = s_core->continue_bw_search(p->getPrimitive(), right_result.range);
                if(!isEmpty(range))
            ...
            case RegExPrimitiveType::Dot:
            {
                for(char c: std::string(s_core->getAlphabet())) {
                    if(c == '\n' || c == (char)1) continue;
                    Range range = s_core->continue_bw_search(std::string(1, c), right_result.range);
                    if(!isEmpty(range))
