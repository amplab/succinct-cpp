...
public:
    AdaptiveManagementClient(std::string master_ip = "localhost") {
        ...
        fprintf(stderr, "AdaptiveManagementClient: Master gave client_id %d and handler %s\n", client_id, handler.c_str());

        // Connect to handler
        mgmt_client = get_client(handler, port, mgmt_transport);

        fprintf(stderr, "AdaptiveManagementClient: Asking management client to connect to handlers...\n");
