...
class AdaptiveSuccinctClient {
 public:
  AdaptiveSuccinctClient(std::string master_ip = "localhost", int32_t port =
  QUERY_HANDLER_PORT) {
    ...
    fprintf(stderr,
            "AdaptiveSuccinctClient: Master gave client_id %d and handler %s\n",
    ...
    fprintf(
        stderr,
        "AdaptiveSuccinctClient: Asking request client to connect to handlers...\n");
