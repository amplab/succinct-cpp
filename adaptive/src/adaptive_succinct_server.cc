...
class AdaptiveSuccinctServiceHandler : virtual public AdaptiveSuccinctServiceIf {
 public:
  AdaptiveSuccinctServiceHandler(uint32_t local_host_id, uint32_t num_shards,
                                 std::vector<std::string> hostnames,
                                 bool construct, uint32_t replication,
    ...
    this->qserver_transports =
        new std::vector<boost::shared_ptr<TTransport>>[MAX_CLIENTS];
    this->qhandlers =
        new std::map<uint32_t, AdaptiveSuccinctServiceClient*>[MAX_CLIENTS];
    this->qhandler_transports = new std::map<uint32_t,
        boost::shared_ptr<TTransport>>[MAX_CLIENTS];
  }
  ...
  int32_t start_servers() {
    ...
    for (uint32_t local_shard_id = 0; local_shard_id < num_shards;
        local_shard_id++) {
      ...
      uint32_t global_shard_id = (local_shard_id * num_hosts + local_host_id)
          / replication;
    ...
    for (int i = 0; i < num_hosts; i++) {
      ...
      try {
        if (i == local_host_id) {
          ...
          boost::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
          AdaptiveSuccinctServiceClient *client =
              new AdaptiveSuccinctServiceClient(protocol);
          transport->open();
          fprintf(stderr, "Connected!\n");
          qhandler_transports[client_id].insert(
              std::pair<uint32_t, boost::shared_ptr<TTransport>>(i, transport));
          qhandlers[client_id].insert(
              std::pair<uint32_t, AdaptiveSuccinctServiceClient*>(i, client));
  ...
  int32_t connect_to_local_servers(const int32_t client_id) {
    for (int i = 0; i < num_shards; i++) {
      ...
      try {
        ...
        boost::shared_ptr<TTransport> transport(new TBufferedTransport(socket));
        boost::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
        AdaptiveQueryServiceClient *qsclient = new AdaptiveQueryServiceClient(
            protocol);
        transport->open();
        fprintf(stderr, "Connected!\n");
  ...
  int32_t get_request(const int32_t client_id, const int64_t key) {
    uint32_t primary_shard_id =
        (uint32_t) (key / LayeredSuccinctShard::MAX_KEYS);
    ...
    if (host_id == local_host_id)
      queue_length = get_request_local(client_id, local_shard_id,
                                       key % LayeredSuccinctShard::MAX_KEYS);
    else
      queue_length = qhandlers[client_id].at(host_id)->get_request_local(
          client_id, local_shard_id, key % LayeredSuccinctShard::MAX_KEYS);
  ...
  int64_t get_request_local(const int32_t client_id,
                            const int32_t local_shard_id, const int64_t key) {
  ...
  void get_response(std::string& _return, const int32_t client_id,
    ...
    if (host_id == local_host_id)
      ...
      qhandlers[client_id].at(host_id)->get_response_local(_return, client_id,
                                                           local_shard_id);
  }

  void get_response_local(std::string& _return, const int32_t client_id,
...
static void monitor_queues_local(std::atomic<uint64_t> *local_queue_lengths,
                                 uint64_t num_local_shards) {
  ...
  while (true) {
    ...
    for (uint32_t i = 0; i < num_local_shards; i++) {
      avg_qlens[i] = local_queue_lengths[i] * alpha
          + (1.0 - alpha) * avg_qlens[i];
      output << "\t" << avg_qlens[i];
    }
    output << "\n";
    sleep(delta);
...
static void monitor_queues_global(std::atomic<uint64_t> *global_queue_lengths,
                                  uint64_t num_global_shards) {
...
void print_usage(char *exec) {
  fprintf(
      stderr,
      "Usage: %s [-m mode] [-s num_local_shards] [-p num_primary_shards] [-r replication] [-h hostsfile] [-i hostid]\n",
      exec);
}
...
int main(int argc, char **argv) {
  ...
  int c;
  uint32_t mode = 0, num_local_shards = 1, replication = 1, num_primary_shards =
      1;
  ...
  std::atomic<uint64_t> *local_queue_lengths =
      new std::atomic<uint64_t>[num_local_shards];
  for (uint32_t i = 0; i < num_local_shards; i++) {
    local_queue_lengths[i] = 0;
  }

  std::atomic<uint64_t> *global_queue_lengths =
      new std::atomic<uint64_t>[num_primary_shards * replication];
...
//            num_local_shards, hostnames, construct, replication, local_queue_lengths, global_queue_lengths));
  shared_ptr<AdaptiveSuccinctServiceHandler> handler(
      new AdaptiveSuccinctServiceHandler(local_host_id, num_local_shards,
                                         hostnames, construct, replication,
                                         local_queue_lengths,
                                         global_queue_lengths));
  shared_ptr<TProcessor> processor(
      new AdaptiveSuccinctServiceProcessor(handler));

  std::thread local_queue_monitor_daemon(monitor_queues_local,
                                         local_queue_lengths, num_local_shards);
  std::thread global_queue_monitor_daemon(monitor_queues_global,
                                          global_queue_lengths,
                                          num_primary_shards * replication);
  try {
