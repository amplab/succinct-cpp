...
class QueryServiceNull : virtual public QueryServiceIf {
 public:
  ...
  int32_t Initialize(const int32_t /* id */, const int32_t /* sampling_rate */) {
    int32_t _return = 0;
    return _return;
  }
...
typedef struct _QueryService_Initialize_args__isset {
  _QueryService_Initialize_args__isset() : id(false), sampling_rate(false) {}
  bool id;
  bool sampling_rate;
...
class QueryService_Initialize_args {
 public:
  ...
  QueryService_Initialize_args() : id(0), sampling_rate(0) {
  }
  ...
  void __set_sampling_rate(const int32_t val) {
    sampling_rate = val;
  }
  ...
  bool operator == (const QueryService_Initialize_args & rhs) const
  {
    if (!(id == rhs.id))
      return false;
    if (!(sampling_rate == rhs.sampling_rate))
      return false;
    return true;
  }
  bool operator != (const QueryService_Initialize_args &rhs) const {
...
class QueryServiceClient : virtual public QueryServiceIf {
 public:
  ...
  int32_t Initialize(const int32_t id, const int32_t sampling_rate);
  void send_Initialize(const int32_t id, const int32_t sampling_rate);
...
class QueryServiceMultiface : virtual public QueryServiceIf {
 ...
 public:
  int32_t Initialize(const int32_t id, const int32_t sampling_rate) {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->Initialize(id, sampling_rate);
    }
    return ifaces_[i]->Initialize(id, sampling_rate);
  }
