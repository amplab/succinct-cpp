...
nohup "$bin/shandler" -m 1 -s "$1" -q "$bin/qserver" -h "$SUCCINCT_CONF_DIR/hosts" -r "$SUCCINCT_CONF_DIR/repl" -i "$2" "$SUCCINCT_DATA_PATH/data" -f "$NUM_FAILURES" 2>"$SUCCINCT_LOG_PATH/handler_${2}.log" &
