...
"$sbin/hosts.sh" cd "$SUCCINCT_HOME" \; awk '{ sum += \$1 } END { print sum }' throughput_results_search_get > "$SUCCINCT_RES_PATH/thput"
"$sbin/hosts.sh" cd "$SUCCINCT_HOME" \; rm throughput_results_search_get
